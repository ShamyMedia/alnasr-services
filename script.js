...
async function boot(){
  try{
    ...
    state.data = (j.shops || []).map(i => ({
      ...i,
      phone: cleanNumber(i.phone),
      whatsapp: cleanNumber(i.whatsapp),
  ...
  return /^(https?:|tel:|mailto:|data:image)/i.test(v) ? v : "";
};


    render();
  }catch(e){
    list.innerHTML = `<div class="loading">حدث خطأ في تحميل البيانات</div>`;
    console.error(e);
  }
}
